...
import {WorkContainer, WorkDescription, WorkItem, WorkLink, WorkTitle, Title, TechStackLabel } from './work.styles.js';

const WorkComponent = () => {
  const recentWork = [
    {
      title: "Cooper Valuation Group Website",
    ...
    <WorkContainer id="work">
      ...
      {recentWork.map((work, index) => (
        <WorkItem key={index}>
          ...
          <WorkDescription> {work.techStack }</WorkDescription>
          
          <WorkDescription>{work.description}</WorkDescription>
          
          {work.link ? ( <WorkLink href={work.link} target="_blank" rel="noopener noreferrer">
            Visit Project
          </WorkLink>):null}
